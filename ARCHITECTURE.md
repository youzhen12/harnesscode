...
6. 创建 `input/prd` 和 `input/techspec` 目录，作为 PRD / 技术规格输入目录
7. 在项目根创建 `docs/` 目录，并在 `.harnesscode/` 下创建 `.harnesscode/docs/` 目录，用于存放和消费技术方案/设计文档
8. 更新项目根 `.gitignore`，忽略 `.harnesscode/`、`dev-log.txt`、`cycle-log.txt`
...
- `config.yaml`：项目配置
- `project_id`：项目 ID
- `feature_list.json`：功能/任务执行文档（由 initializer 基于技术方案和 PRD 构建，驱动整个循环）
- `missing_info.json`：缺失信息 / 问题列表
- `claude-progress.txt`：Agent 的文字进度记录
- `docs/`：为 AI Agent 消费的技术方案/设计文档（通常由你从项目根 `docs/` 或其它资料拷贝/整理而来）
...
- **orchestrator**（协调者）
  - 输入：
    ...
    - `.harnesscode/docs/` 下的技术方案/设计文档（视为当前开发计划的主来源）
    - `.harnesscode/feature_list.json`（执行文档）
    - `.harnesscode/claude-progress.txt`
    - 其它 `.harnesscode/*` 状态文件（如 `test_report.json`、`review_report.json` 等，未来可扩展）
  - 任务：
    - 确保存在一份与 `.harnesscode/docs/` 对齐的执行文档：`feature_list.json`；如缺失或明显不一致，则调度 `initializer` 去解析技术方案并构建/更新 feature_list
    - 在 feature_list 就绪后，为每个来自技术方案的 feature 驱动一个流水线：
      1. 调用 `coder` 按技术方案和 feature_list 实现/优化代码；
      2. 调用 `tester` 为相关 API/行为创建/更新测试并运行测试；
      3. 如测试失败，调用 `fixer` 进行修复，然后再次调用 `tester` 重跑测试；
      4. 测试通过后，调用 `reviewer` 审查近期改动，如有阻塞问题，再调用 `fixer` 解决，并按需回到 `tester/reviewer`；
    - 当所有相关 feature 均标记为 `completed`，测试通过且审查无阻塞时，才允许输出 `complete` 结束循环
  - 输出：
...
- **initializer**（特性初始化/同步）
  - 输入：
    ...
    - `.harnesscode/claude-progress.txt`
    - PRD / 技术文档：`input/prd/`、`input/techspec/`、项目根 `docs/`、以及 `.harnesscode/docs/` 下的技术方案文件
  - 任务：
    - 从上述文档中识别出功能/任务，并尽量映射到真实的代码或测试改动点（避免只生成占位特性）
    - 建立或更新 `.harnesscode/feature_list.json`，使其成为与技术方案对齐的“执行文档”，并尽量保持 feature ID 稳定
    - 简要记录本次同步的内容和使用的文档到 `claude-progress.txt`
  - 输出：
